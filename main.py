...
def add():
    ...
    li.delete(0, END)
    li.insert(END, (
        date_text.get(), earning_text.get(), exercise_text.get(), study_text.get(), diet_text.get(), others_text.get()))


win = Tk()

win.wm_title('ROUTINE DATABASE')
...
others_text = StringVar()
e6 = Entry(win, textvariable=others_text)
e6.grid(row=2, column=3)

li = Listbox(win, height=8, width=35)
li.grid(row=3, column=0, rowspan=9, columnspan=2)
...
b1 = Button(win, text='ADD', width=12, pady=5, command=add)
b1.grid(row=3, column=3)
b2 = Button(win, text='SEARCH', width=12, pady=5, command=search_command)
b2.grid(row=4, column=3)
b3 = Button(win, text='DELETE DATA', width=12, pady=5, command=delete_command)
